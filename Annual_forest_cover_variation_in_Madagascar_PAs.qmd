...
Our assignment was to calculate the forest cover and annual forest cover loss of all terrestrial protected areas in Madagascar and also at national level. These calculations are intended to serve as an input for [an](https://www.kfw-entwicklungsbank.de/International-financing/KfW-Development-Bank/Evaluations/Results/) [*ex post* evaluation](https://www.kfw-entwicklungsbank.de/International-financing/KfW-Development-Bank/Evaluations/Results/) of KfW support to conservation in this country.
...
**Third limitation: inaccurate geometries**. The spatial perimeters of protected areas are sometimes inaccurate, as demonstrates the comparison below with the (apparently more precise) Vahatra data. This means that statistics on forest cover or forest cover loss based on WDPA might includes areas that are in fact outside the protected areas perimeters.
...
From this, we would recommend to use the Vahatra data instead of the WDPA data (see visualization below). The data from Vahatra is available on [a dedicated portal](https://protectedareas.mg) that makes it accessible under a creative commons licence ([CC-BY](https://creativecommons.org/licenses/by/2.0/)).

```{r}
#| fig-subcap: "Source : Association Vahatra; Goodman et al. 2018."
tmap_mode("view")
tm_shape(contour_mada) +
  tm_borders() +
...
tm_shape(footprint_treecover_tiles) +
  ...
  tm_layout(title = "No results for polygons overlapping tile limits",
